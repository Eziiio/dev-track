...
function Dashboard() {
  ...
  const getStatusBadgeClass = (status) => {
    switch (status) {
      ...
      case 'Review':
        return 'bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 border-indigo-200/50 dark:border-indigo-800/50';
      default:
  ...
  const getActivityIconStyles = (action) => {
    if (action.includes('created')) {
      return {
        bg: 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800/40',
        dot: 'bg-emerald-500'
      };
    }
    if (action.includes('deleted')) {
      return {
        bg: 'bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-800/40',
        dot: 'bg-rose-500'
      };
    }
    return {
      bg: 'bg-primary-50 dark:bg-primary-950/40 text-primary-600 dark:text-primary-400 border-primary-100 dark:border-primary-800/40',
      dot: 'bg-primary-500'
    };
  };
    ...
    return (
      <div className="space-y-8 animate-pulse">
          ...
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-28 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-3">
              <div className="flex justify-between">
                <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-850" />
                <div className="w-6 h-4 rounded bg-slate-100 dark:bg-slate-850" />
              </div>
              <div className="w-14 h-7 rounded bg-slate-100 dark:bg-slate-850" />
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="h-[540px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6" />
        </div>
      </div>
    );
  }

  const { totals = {}, projectProgress = [], upcomingDeadlines = [], recentActivities = [] } = data || {};
  ...
  return (
    <div className="space-y-8">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-xl border ${card.color.split(' ')[1]} ${card.color.split(' ')[2]} ${card.color.split(' ')[3]}`}>
      ...
      {/* 2. Grid contents: Project progress + upcoming deadlines + Activities timeline */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Left Columns (Project progress + upcoming deadlines) */}
        <div className="lg:col-span-2 space-y-6">
          
          {/* Project Progress list */}
          <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
            <h2 className="font-display text-lg font-bold text-slate-850 dark:text-white mb-6 flex items-center gap-2">
            ...
            {projectProgress.length === 0 ? (
              <div className="text-center py-8 space-y-2.5">
                <FolderKanban size={36} className="text-slate-300 mx-auto" />
                <p className="text-slate-400 text-sm font-medium">No projects found in this workspace.</p>
              </div>
            ) : (
                ...
                {projectProgress.map((project) => (
                  <div key={project._id} className="space-y-2">
                    <div className="flex justify-between items-center text-sm font-semibold">
                      <span className="text-slate-700 dark:text-slate-200 truncate pr-4">
                        {project.title}
                      </span>
          ...
          <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
            ...
            {upcomingDeadlines.length === 0 ? (
              <div className="text-center py-8 space-y-2.5">
                <CheckSquare size={36} className="text-slate-300 mx-auto" />
                <p className="text-slate-400 text-sm font-medium">No upcoming task deadlines.</p>
              </div>
            ) : (
                ...
                {upcomingDeadlines.map((task) => (
                  ...
                  >
                    <div className="space-y-1.5 min-w-0">
                      <p className="font-semibold text-slate-800 dark:text-slate-200 truncate text-sm sm:text-base">
                        {task.title}
                      </p>
                      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-400 dark:text-slate-500">
                        <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200/30">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                      <div className="flex gap-2">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${getStatusBadgeClass(task.status)}`}>
                          {task.status}
                        </span>
                      </div>
                      ...
                      {/* Assignee avatar */}
                      {task.assignedTo ? (
                        <div className="w-8 h-8 rounded-xl bg-primary-100 text-primary-700 font-bold flex items-center justify-center text-xs border border-primary-200/20" title={`Assigned to ${task.assignedTo.name}`}>
                          {task.assignedTo.name.charAt(0).toUpperCase()}
                        </div>
                      ) : (
                        <div className="w-8 h-8 rounded-xl bg-slate-100 text-slate-400 flex items-center justify-center text-xs border border-slate-200/50" title="Unassigned">
                          <User size={14} />
                        </div>
                      )}
                    </div>
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col h-[540px]">
          <h2 className="font-display text-lg font-bold text-slate-850 dark:text-white mb-6 flex items-center gap-2 shrink-0">
            <ActivityIcon className="text-primary-500" size={18} />
            Recent Activities
          </h2>

          <div className="flex-1 overflow-y-auto pr-1 space-y-6 relative border-l border-slate-150 dark:border-slate-800/80 ml-3.5 pl-5">
            {recentActivities.length === 0 ? (
              <div className="text-center py-16 space-y-2.5 -ml-5">
                <ActivityIcon size={36} className="text-slate-300 mx-auto" />
              ...
              recentActivities.map((act) => {
                ...
                return (
                  <div key={act._id} className="relative group text-sm">
                    {/* Timeline icon dot */}
                    <span className={`absolute -left-[27.5px] top-0.5 w-3.5 h-3.5 rounded-full border-2 border-white dark:border-slate-900 flex-shrink-0 flex items-center justify-center ${colors.dot}`} />
                    ...
                    <div className="space-y-1">
                      <p className="text-slate-600 dark:text-slate-300 font-medium leading-relaxed">
                        ...
                        {act.action}
                        {act.task && (
                          <span className="font-semibold text-slate-700 dark:text-slate-200 block sm:inline">
                            {' '}"{act.task.title}"
                          </span>
                        )}
                        {act.project && (
                          <span className="text-xs text-slate-400 dark:text-slate-500 block">
                            in {act.project.title}
                          </span>
                        )}
                      </p>
